> **当前验证基线（0.9.6）**：Swift 43 项、Android 49 项通过，Android lint、APK 和 Mac release 构建通过。实际验证连接小米、启动鼠标捕获、密码框拒绝时保持捕获及 Esc 退出。下方各版本记录是历史结果，其中“尚未实现”等表述仅适用于对应版本。当前能力见[架构](architecture.md)，待验收项目见[开发清单](development.md#实机验收清单)。
...
## Android 0.9.6 — 输入法面板入口

输入法面板除「切换输入法」按钮外可点击进入 MainActivity，复用已有任务。Android 49 项测试、lint 与 assembleDebug 通过；面板跳转仍需手机覆盖安装后验收，未声称已完成真机点击测试。
