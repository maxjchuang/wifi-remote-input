...
class InputServer(address: InetSocketAddress, tls: SSLContext, private val pairing: Pairing, private val input: (String, String, () -> Boolean) -> String, private val status: (String) -> Unit, private val snapshot: (() -> Boolean) -> org.json.JSONObject = { org.json.JSONObject().put("status", "snapshot_unavailable") }, private val deviceName: () -> String = { "Android 手机" }, private val peersChanged: (Set<String>) -> Unit = {}, private val controlClosed: (String) -> Unit = {}) : WebSocketServer(address, 2, listOf(Draft_6455(listOf(DefaultExtension()), listOf(WireProtocol("")), 16_384))) {
    private val sessions = ConcurrentHashMap<WebSocket, Protocol>()
    private val owners = ConcurrentHashMap<WebSocket, String>()
    ...
    override fun onOpen(conn: WebSocket, handshake: ClientHandshake) {
        ...
        protocol = Protocol(pairing, snapshot = { snapshot { protocol.credential?.let(pairing::authenticate) == true } }, deviceName = deviceName) { type, value -> input(type, if (type == "control.action") (if (value.startsWith("{")) org.json.JSONObject(value) else org.json.JSONObject().put("action", value)).put("owner", owner).toString() else value) { sessions[conn] === protocol && protocol.credential?.let(pairing::authenticate) == true } }
    ...
    override fun onClose(conn: WebSocket, code: Int, reason: String, remote: Boolean) { sessions.remove(conn); owners.remove(conn)?.let(controlClosed); publishPeers() }
