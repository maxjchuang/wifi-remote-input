...
class InputServer(address: InetSocketAddress, tls: SSLContext, private val pairing: Pairing, private val input: (String, String, () -> Boolean) -> String, private val status: (String) -> Unit) : WebSocketServer(address, 2, listOf(Draft_6455(listOf(DefaultExtension()), listOf(WireProtocol("")), 16_384))) {
    ...
    override fun onOpen(conn: WebSocket, handshake: ClientHandshake) {
        ...
        protocol = Protocol(pairing) { type, value -> input(type, value) { protocol.credential?.let(pairing::authenticate) == true } }
    ...
    override fun onClose(conn: WebSocket, code: Int, reason: String, remote: Boolean) { sessions.remove(conn) }
